...
export default function Index() {
    const [url, setUrl] = useState('http://ws.audioscrobbler.com/2.0/?method=album.search&album=believe&api_key=e40e5d1c2779cb5d11f90299890b38ac&format=json');
    const { data, error } = useSWR(url, theFetcher);

    const onClickHandler = (e) => {
        e.preventDefault();
        if (url === '') setUrl('http://ws.audioscrobbler.com/2.0/?method=album.search&album=believe&api_key=e40e5d1c2779cb5d11f90299890b38ac&format=json');
        else setUrl('');
    }
                        ...
                        <div>
                            {data.Search.map((m) => (
                                ...
                                <Popover content={(content) => {
                                    ...
                                    <Button style={{ display: "block", margin: "1rem auto" }}><Link href={`/music/${m.imdbID}`}>
                                        {m.Title} ({m.Year})
                                    </Link></Button>
                                </Popover>
                            ))}
